...
function ChallengesPage() {
  ...
  return (
    <div className="min-h-screen" style={{ background: 'var(--base-bg)' }}>
      {/* Toast */}
      {toast && (
        <div className="fixed top-6 right-6 z-50 px-5 py-3 rounded-xl shadow-lg text-sm font-semibold text-primary-inverse"
          style={{ background: toast.type === 'error' ? '#EF4444' : 'var(--primary)' }}>
          {toast.msg}
        </div>
      )}
      ...
      <div className="py-16 px-4" style={{ background: 'linear-gradient(135deg, var(--primary), var(--secondary))' }}>
        <div className="max-w-4xl mx-auto text-center">
          ...
          <h1 className="text-4xl md:text-5xl font-bold text-primary-inverse mb-4 leading-tight">
            Transform your life,<br />
            <span style={{ color: 'var(--accent)' }}>one day at a time</span>
          </h1>
          <p className="text-primary-inverse opacity-70 text-lg max-w-2xl mx-auto">
          ...
          <div className="flex justify-center gap-8 mt-10">
            {[
              ...
              <div key={i} className="text-center">
                <div className="text-2xl font-bold text-primary-inverse">{loading ? '—' : s.value}</div>
                <div className="text-xs text-primary-inverse opacity-50 mt-1">{s.label}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* Filter bar */}
      <div className="sticky top-0 z-10 py-3 px-4 border-b" style={{ background: 'var(--base-bg)', borderColor: '#D4DBC8' }}>
          ...
          {['all', 'joined', 'new'].map(f => (
            <button key={f} onClick={() => setFilter(f)}
              ...
              style={filter === f
                ...
                : { background: 'color-mix(in srgb, var(--base-bg) 80%, var(--primary) 20%)', color: 'var(--primary)' }}>
      ...
      <div className="max-w-6xl mx-auto px-4 py-10">
        {loading ? (
          <div className="grid md:grid-cols-2 gap-6">
            {[1,2,3,4].map(i => (
              <div key={i} className="rounded-2xl h-64 animate-pulse" style={{ background: 'color-mix(in srgb, var(--base-bg) 80%, var(--primary) 20%)' }} />
            ))}
          </div>
        ) : filteredChallenges.length === 0 ? (
          <div className="text-center py-20">
            <Leaf className="w-12 h-12 mx-auto mb-4 opacity-20" style={{ color: 'var(--primary)' }} />
            ...
            {filteredChallenges.map((challenge, idx) => {
              ...
              return (
                <div key={challenge.id}
                  ...
                  {/* Card header */}
                  <div className="p-6" style={{ background: 'linear-gradient(135deg, var(--primary), var(--secondary))' }}>
                    <div className="text-4xl mb-3">{ICONS[idx % ICONS.length]}</div>
                    <h3 className="text-xl font-bold text-primary-inverse mb-1">{challenge.title}</h3>
                    <p className="text-primary-inverse opacity-70 text-sm">{challenge.description}</p>
                  </div>
                    ...
                    {isJoined && (
                      <div className="mb-4">
                        <div className="flex items-center justify-between text-xs mb-1.5">
                          ...
                          <span style={{ color: 'var(--primary)' }} className="font-bold">{pct}%</span>
                        </div>
                        <div className="w-full h-2 rounded-full" style={{ background: 'color-mix(in srgb, var(--base-bg) 80%, var(--primary) 20%)' }}>
                    ...
                    <div className="flex items-center gap-3">
                      {isJoined ? (
                        ...
                        <button onClick={() => handleJoin(challenge.id)}
                          ...
                          className="flex items-center gap-2 px-5 py-2.5 rounded-full text-sm font-semibold text-primary-inverse transition-opacity disabled:opacity-60"
                      ...
                      <span className="ml-auto flex items-center gap-1 text-xs" style={{ color: 'var(--secondary)' }}>
        ...
        {/* Why section */}
        <div className="mt-16 rounded-3xl p-10 text-center" style={{ background: 'linear-gradient(135deg, var(--primary), var(--secondary))' }}>
          <h2 className="text-2xl font-bold text-primary-inverse mb-2">Why join a wellness challenge?</h2>
          <p className="text-primary-inverse opacity-60 text-sm mb-8 max-w-xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {[
              ...
              <div key={i} className="rounded-2xl p-6 text-left" style={{ background: 'rgba(255,255,255,0.07)' }}>
                ...
                <h3 className="font-bold text-primary-inverse mb-1">{title}</h3>
                <p className="text-primary-inverse opacity-50 text-sm">{desc}</p>
              </div>
            ))}
          </div>
        </div>
