...
const CATEGORY_META = {
  ...
  Safety:    { color: 'var(--accent)', bg: '#FDF6F0', label: 'Safety & Abuse' },
  Substance: { color: 'var(--primary)', bg: 'color-mix(in srgb, var(--base-bg) 80%, var(--primary) 20%)', label: 'Substance Support' },
  Eating:    { color: 'var(--primary)', bg: '#EFF5E8', label: 'Eating Disorders' },
}
...
function FreeHelpPage() {
  ...
  return (
    <div className="min-h-screen" style={{ background: 'var(--base-bg)' }}>
      {/* Hero */}
      <div className="py-16 px-4" style={{ background: 'linear-gradient(135deg, var(--primary), var(--secondary))' }}>
        <div className="max-w-4xl mx-auto text-center">
          ...
          <h1 className="text-4xl md:text-5xl font-bold text-primary-inverse mb-4">
            You are not alone
          </h1>
          <p className="text-primary-inverse opacity-70 text-lg max-w-2xl mx-auto mb-8">
          ...
          <div className="inline-flex items-center gap-3 px-6 py-4 rounded-2xl border text-left"
            ...
            <div>
              <p className="font-bold text-primary-inverse text-sm">In immediate danger?</p>
              <p className="text-primary-inverse opacity-70 text-xs">Call 911 or go to your nearest emergency room now</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-4 py-10 space-y-12">
        ...
        {/* 24/7 Crisis row */}
        <section>
          <h2 className="text-xl font-bold mb-5" style={{ color: 'var(--base-text)' }}>24/7 Crisis Hotlines</h2>
          <div className="grid sm:grid-cols-3 gap-4">
            {[
              ...
              <div key={i} className="rounded-2xl p-6 flex flex-col gap-2"
                ...
                <Phone className="w-6 h-6 text-primary-inverse opacity-80" />
                <p className="text-primary-inverse font-semibold text-sm">{h.name}</p>
                <a href={`tel:${h.number.replace(/\D/g,'')}`}
                  className="text-primary-inverse text-2xl font-bold hover:opacity-80">{h.number}</a>
                <p className="text-primary-inverse opacity-60 text-xs">{h.desc}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <section>
          <h2 className="text-xl font-bold mb-5" style={{ color: 'var(--base-text)' }}>Support Resources</h2>
          {loading ? (
            ...
            <div className="space-y-6">
              {Object.entries(grouped).map(([cat, items]) => {
                const meta = CATEGORY_META[cat] || { color: 'var(--primary)', bg: 'color-mix(in srgb, var(--base-bg) 80%, var(--primary) 20%)', label: cat }
                return (
                  <div key={cat}>
                    ...
                    <div className="grid sm:grid-cols-2 gap-3">
                      {items.map(r => (
                        <div key={r.id} className="flex items-center gap-4 rounded-2xl border p-4"
                          style={{ background: '#fff', borderColor: 'color-mix(in srgb, var(--base-bg) 80%, var(--primary) 20%)' }}>
                          <div className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0"
                            style={{ background: meta.bg }}>
                            <Phone className="w-4 h-4" style={{ color: meta.color }} />
                          </div>
                          <div className="flex-1 min-w-0">
                            <p className="font-semibold text-sm truncate" style={{ color: 'var(--base-text)' }}>{r.name}</p>
        ...
        {/* AI Chat section */}
        <section>
          <div className="rounded-3xl overflow-hidden border" style={{ borderColor: 'color-mix(in srgb, var(--base-bg) 80%, var(--primary) 20%)' }}>
            {/* Header */}
            <div className="p-6 flex items-center justify-between cursor-pointer"
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-bold text-primary-inverse">Talk to our AI Companion</p>
                  <p className="text-xs text-primary-inverse opacity-60">Private, judgment-free support anytime</p>
                </div>
              </div>
              <ChevronRight className={`w-5 h-5 text-primary-inverse opacity-60 transition-transform ${chatOpen ? 'rotate-90' : ''}`} />
            </div>
            ...
            {chatOpen && (
              <div style={{ background: '#fff' }}>
                ...
                <div className="h-72 overflow-y-auto p-4 space-y-3">
                  ...
                  {aiLoading && (
                    <div className="flex justify-start">
                      <div className="px-4 py-3 rounded-2xl" style={{ background: 'color-mix(in srgb, var(--base-bg) 80%, var(--primary) 20%)' }}>
                ...
                {/* Input */}
                <div className="flex items-center gap-3 p-4 border-t" style={{ borderColor: 'color-mix(in srgb, var(--base-bg) 80%, var(--primary) 20%)' }}>
                  <input
                    value={input}
                    ...
                    style={{ background: 'var(--base-bg)', borderColor: 'color-mix(in srgb, var(--base-bg) 80%, var(--primary) 20%)', color: 'var(--base-text)' }}
                  />
                  <button onClick={sendMessage}
                    disabled={!input.trim() || aiLoading}
                    className="w-9 h-9 rounded-full flex items-center justify-center transition-opacity disabled:opacity-40"
                    style={{ background: 'var(--primary)' }}>
                    <ChevronRight className="w-4 h-4 text-primary-inverse" />
                  </button>
                </div>
                {!isAuthenticated && (
        ...
        <section className="text-center py-8">
          <Leaf className="w-8 h-8 mx-auto mb-3 opacity-30" style={{ color: 'var(--primary)' }} />
